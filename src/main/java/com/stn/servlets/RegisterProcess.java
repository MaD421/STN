            ...
            try {
                if(userHelper.checkAvailability(username, email)) { //daca nu avem rezultate,inseamna ca user-ul sau parola nu exista in baza de date
                   //Introducere utilizator in baza de date
                    ...
                    if(invitatie != null) {
                        int id = userHelper.addUser(username,hashedPassword,salt,email,firstName,lastName,invitatie.getIdGrupa(),invitatie.getIdSerie(),invitatie.getIdFacultate(),invitatie.getUserClass());

                        String text = "New user account <a target='_blank' href='/userdetails.jsp?id="+
                                id+"' style='text-decoration: none'><b style='color: "+userHelper.classColor(invitatie.getUserClass())+"'>"+username+
                                "</b></a> from <a target='_blank' class='ui' href='https://db-ip.com/"+ip+"' style='color: #7ADC94;'>"+ip+"</a> was created.";
                        logHelper.addLog(text,2);

                        applicationHelper.deleteInvitatie(invitatie.getIdInvitatie());
                    } else {
                        int id = userHelper.addUser(username,hashedPassword,salt,email,firstName,lastName,0,0,0,1);

                        String text = "New user account <a target='_blank' href='/userdetails.jsp?id="+
                                id+"' style='text-decoration: none'><b style='color: "+userHelper.classColor(1)+"'>"+username+
                                "</b></a> from <a target='_blank' class='ui' href='https://db-ip.com/"+ip+"' style='color: #7ADC94;'>"+ip+"</a> was created.";
                        logHelper.addLog(text,2);
                    }
